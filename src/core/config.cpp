	...
	while( fgets( linebuffer, 0xFF, fp ) != NULL && !feof(fp) ){
		...
		if( linebuffer[0] != 0x00 && linebuffer[0] != '#' ){
			...
			*p++ = 0x00;
			...
			if( m_map.find(namebuffer) == m_map.end() ){
				m_map[namebuffer] = confbuffer;
			}
			// we consider duplicated keys as appending to a one CSV string
			else{
				m_map[namebuffer] += string(",") + confbuffer;
			}
		}
		++lineno;
...
void Configuration::getMultiple( char *name, vector<string>& values ){
	map<string,string>::iterator i;
	string						 csv;
	...
	if( i == m_map.end()  ){
		...
		string::size_type lastPos = csv.find_first_not_of( ",", 0 );
   		string::size_type pos     = csv.find_first_of( ",", lastPos );

		while( string::npos != pos || string::npos != lastPos ){
			values.push_back( csv.substr( lastPos, pos - lastPos ) );
			
			lastPos = csv.find_first_not_of( ",", pos ) ;
			pos 	= csv.find_first_of( ",", lastPos );
		}
	}
}
